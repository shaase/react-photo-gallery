  ...
  scalePhotoDimensions() {
    ...
    rows = rows.map(row => {
      const totalAspectRatio = row.reduce(
        (acc, photo, idx) => acc + this.aspectRatio(photo),
        0
      );
      ...
      const rowWidth =
        row.length < cols
          ? Math.floor(
              containerWidth / cols * row.length - row.length * (margin * 2)
            )
    ...
    return (
      <div id="Gallery" className="clearfix" ref={c => (this._gallery = c)}>
        {resizedPhotos.map((photo, idx) => {
          const srcset = photo.srcset !== undefined ? photo.srcset.join : [];
          const sizes = photo.sizes !== undefined ? photo.sizes.join : [];
          return (
